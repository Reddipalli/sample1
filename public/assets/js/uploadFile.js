...
$(document).ready(function() {
    var dn_arr = document.getElementsByClassName('file_plugin');
    for (var i = 0; i < dn_arr.length; i++) {
        ...
        var html = "<label class ='file-input' for='file-input'><i class='fa fa-paperclip fa-lg' aria-hidden='true'></i></label><input  class='hide' id='file-input' autocomplete='off' type='file' name='" + dname + "' onchange='uploadFile(\"" + dname + "\")' />";
...
function uploadFile(name) {
    ...
    getBase64(file).then(
        data => {
            ...
            $.post('/uploadfiles', { data: filedata }, function(resdata) {
